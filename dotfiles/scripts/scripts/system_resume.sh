...
save)
  ...
  ADAPTER=$(busctl tree org.bluez | grep -o '/org/bluez/hci[0-9]*' | head -n1)

  if [ -n "$ADAPTER" ] && busctl get-property org.bluez "$ADAPTER" org.bluez.Adapter1 Powered | grep -q true; then
